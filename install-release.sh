...
identify_the_operating_system_and_architecture() {
  if [[ "$(uname)" == 'Linux' ]]; then
    ...
    ### Refer: https://github.com/oceantide/fhs-install-v2ray/issues/84#issuecomment-688574989
...
get_version() {
  ...
  TMP_FILE="$(mktemp)"
  if ! curl -x "${PROXY}" -sS -H "Accept: application/vnd.github.v3+json" -o "$TMP_FILE" 'https://api.github.com/repos/oceantide/v2ray-core/releases/latest'; then
    "rm" "$TMP_FILE"
